...
\author{Juan Ignacio Fernandez \and [Nombre 2]}
...
\begin{frame}{QFT vs.\ DFT/FFT clásica}
  ...
  \begin{tabular}{lll}
    ...
    Sobre qué actúa
      ...
      & Amplitudes de $n=\log_2 M$ qubits \\[3pt]
    Costo
      & FFT: $O(M\log M)=O(2^n\cdot n)$
      & $O(n^2)=O((\log M)^2)$ compuertas \\[3pt]
    Salida
      & Todas las $M$ componentes, \textbf{legibles}
      & Una sola muestra (probabilística) \\[3pt]
    Acceso
...
\begin{frame}{Crecimiento comparativo (intuición)}
  ...
  \begin{tikzpicture}[scale=0.85]
    ...
    % GNFS (sub-exponencial) — curva que sube rápido
    \draw[cuantico, very thick, domain=0.3:7, samples=60]
      plot (\x, {0.08*exp(1.15*\x^(0.5))})
      node[right]{\small\textcolor{cuantico}{GNFS (clásico)}};

    % Shor (polinómico) — curva que crece suave
    \draw[clasico, very thick, domain=0.5:7, samples=60]
      plot (\x, {0.04*\x^2.2})
      node[right]{\small\textcolor{clasico}{Shor (cuántico)}};
...
\begin{frame}{Referencias}
  {\small
  \textbf{Fuente técnica estándar:}
  \begin{itemize}
    \item M.\ Nielsen, I.\ Chuang, \textit{Quantum Computation and Quantum
          Information}, Cambridge — cap.\ 5 y Apéndice A4 (Teorema A4.13).
    \item P.\ Shor, ``Algorithms for Quantum Computation\ldots'',
          FOCS 1994.
  \end{itemize}

  \textbf{Material de la cátedra:}
  \begin{itemize}
    \item C.\ H.\ Ugwuishiwu et al., ``An Overview of Quantum Cryptography
          and Shor's Algorithm'', \textit{IJATCSE} 9(5), 2020.
    \item Clase 04 — Criptografía: Cifrado asimétrico y firma digital, ITBA.
  \end{itemize}

  \textbf{Hardware y estimaciones (consultado junio 2026):}
  \begin{itemize}
    \item Google Quantum AI, chip ``Willow'', 2024--2025.
    \item IBM, hoja de ruta ``Starling 2029'', comunicado 10-jun-2025.
    \item C.\ Gidney, arXiv:2505.15917, 2025 ($<$1M qubits físicos).
    \item C.\ Gidney, M.\ Ekerå, 2019 (20M qubits físicos, 8h).
  \end{itemize}

  \textbf{NIST PQC (consultado junio 2026):}
  \begin{itemize}
    \item FIPS 203/204/205 finalizados 13-ago-2024; HQC seleccionado 11-mar-2025.
  \end{itemize}
  }
