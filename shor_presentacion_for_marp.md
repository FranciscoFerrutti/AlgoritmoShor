...
<style>
section { font-size: 20px; line-height: 1.45; padding: 36px 50px; }
section h1 { font-size: 1.7em; }
section h2 { font-size: 1.3em; margin-bottom: 0.35em; }
section h3 { font-size: 1.15em; }
section h4 { font-size: 1.05em; }
section p, section li { line-height: 1.4; }
section pre { font-size: 0.78em; line-height: 1.25; }
section :not(pre) > code { font-size: 0.92em; }
section table { font-size: 0.9em; }
section blockquote { font-size: 0.95em; }
section.tight { font-size: 17px; }
section.tighter { font-size: 14px; }
section.dense { font-size: 12px; }
</style>
...
<!-- Presenter notes (no se renderizan en la diapositiva):
**Notas de orador:**
...
## Motivación: por qué nos importa
...
## Por qué factorizar N rompe RSA
...
## El problema real: factorización ≡ order-finding
...
## Prerrequisitos sobre el input N
...
## El corazón: el período de f(x) = k^x mod N
...
## Esqueleto del algoritmo: clásico vs. cuántico
...
## GCD y Euclides: honestidad sobre su uso
...
## Por qué el período permite factorizar (1/5): el punto de partida
...
## (b) Por qué se exige k^(r/2) + 1 ≢ 0 (mod N)
...
## (c) Por qué gcd(k^(r/2) ± 1, N) da factores no triviales
...
## (d) "Alta probabilidad": la cota cuantitativa
...
## La parte cuántica: hallar el período
...
## Estimación de fase: el motor del order-finding
...
## El circuito cuántico completo (diagrama)
...
## El bloque costoso: exponenciación modular controlada
...
## De la medición al período: fracciones continuas
...
## Caso k = 7 — parte cuántica: la periodicidad
...
## Caso k = 7 — mediciones y fracciones continuas
...
## Complejidad: Shor vs. el mejor clásico (GNFS)
...
## Crecimiento comparativo (intuición)
...
## Estado actual del hardware cuántico (2025–2026)
...
## El "gap" honesto: lo más grande factorizado de verdad
...
## Estimaciones de recursos para romper RSA-2048
...
## Qué hacer hoy: "harvest now, decrypt later"
